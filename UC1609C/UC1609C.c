...
void UC1609C_DrawFillRoundRect(int16_t x, int16_t y, uint16_t width, uint16_t height, int16_t cornerRadius, uint8_t color) {
  ...
  UC1609C5_DrawRectangleFilled(x + cornerRadius, y, x + cornerRadius + width - 2 * cornerRadius, y + height, color);
  // draw four corners
  UC1609C_DrawFillCircleHelper(x + width - cornerRadius - 1, y + cornerRadius, cornerRadius, 1, height - 2 * cornerRadius - 1, color);
  UC1609C_DrawFillCircleHelper(x + cornerRadius, y + cornerRadius, cornerRadius, 2, height - 2 * cornerRadius - 1, color);
}
//==============================================================================
...
void UC1609C_DrawFillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t corners, int16_t delta, uint8_t color) {
  ...
  while (x < y) {
    ...
    if (x < (y + 1)) {
      if (corners & 1){
        UC1609C_DrawLine(x0 + x, y0 - y, x0 + x, y0 - y - 1 + 2 * y + delta, color);
			}
      if (corners & 2){
        UC1609C_DrawLine(x0 - x, y0 - y, x0 - x, y0 - y - 1 + 2 * y + delta, color);
			}
    ...
    if (y != py) {
      if (corners & 1){
        UC1609C_DrawLine(x0 + py, y0 - px, x0 + py, y0 - px - 1 + 2 * px + delta, color);
			}
      if (corners & 2){
        UC1609C_DrawLine(x0 - py, y0 - px, x0 - py, y0 - px - 1 + 2 * px + delta, color);
			}
...
void UC1609C_DrawCircleHelper(int16_t x0, int16_t y0, int16_t radius, int8_t quadrantMask, uint8_t color)
{
    ...
    while (x <= y) {
        ...
        if (quadrantMask & 0x4) {
            UC1609C_DrawPixel(x0 + x, y0 + y, color);
            UC1609C_DrawPixel(x0 + y, y0 + x, color);;
        }
        if (quadrantMask & 0x2) {
						UC1609C_DrawPixel(x0 + x, y0 - y, color);
            UC1609C_DrawPixel(x0 + y, y0 - x, color);
...
void UC1609C_DrawRoundRect(int16_t x, int16_t y, uint16_t width, uint16_t height, int16_t cornerRadius, uint8_t color) {
  ...
  UC1609C_DrawLine(x + cornerRadius, y, x + cornerRadius + width -1 - 2 * cornerRadius, y, color);         // Top
  UC1609C_DrawLine(x + cornerRadius, y + height - 1, x + cornerRadius + width - 1 - 2 * cornerRadius, y + height - 1, color); // Bottom
  UC1609C_DrawLine(x, y + cornerRadius, x, y + cornerRadius + height - 1 - 2 * cornerRadius, color);         // Left
  UC1609C_DrawLine(x + width - 1, y + cornerRadius, x + width - 1, y + cornerRadius + height - 1 - 2 * cornerRadius, color); // Right
	
  // draw four corners
	UC1609C_DrawCircleHelper(x + cornerRadius, y + cornerRadius, cornerRadius, 1, color);
  UC1609C_DrawCircleHelper(x + width - cornerRadius - 1, y + cornerRadius, cornerRadius, 2, color);
	UC1609C_DrawCircleHelper(x + width - cornerRadius - 1, y + height - cornerRadius - 1, cornerRadius, 4, color);
  UC1609C_DrawCircleHelper(x + cornerRadius, y + height - cornerRadius - 1, cornerRadius, 8, color);
}
//==============================================================================
...
void UC1609C_DrawLineThick(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint8_t color, uint8_t thick) {
	...
	if (thick > 1){
		UC1609C_DrawCircleFilled(x2, y2, thick >> 1, color);
	}
	else{
		UC1609C_DrawPixel(x2, y2, color);
	}

	while (x1 != x2 || y1 != y2) {
		if (thick > 1){
			UC1609C_DrawCircleFilled(x1, y1, thick >> 1, color);
		}
		else{
			UC1609C_DrawPixel(x1, y1, color);
		}
...
//==============================================================================
void ST7789_DrawArc(int16_t x0, int16_t y0, int16_t radius, int16_t startAngle, int16_t endAngle, uint8_t color, uint8_t thick) {
	...
	for (int16_t angle = startAngle; angle <= endAngle; angle += 2) {
		...
		if (thick > 1){
			UC1609C_DrawLineThick(xLast, yLast, x, y, color, thick);
		}
		else{
			UC1609C_DrawLine(xLast, yLast, x, y, color);
		}
